...
class Mailing:
    ...
    async def __send_copy_message(self, from_id: int, chat_id: int, message_id: int, keyboard) -> None:
        try:
            ...
            self.count_blocks += 1
            self.blocked_users.append(from_id)
    ...
    async def __preparing_chunks(self, users: list, chat_id: int, message_id: int, keyboard):
        ...
        for num, user, in enumerate(users):
            task = self.__send_copy_message(user.get('user_id'), chat_id, message_id, keyboard)
            tasks.append(task)
    ...
    async def run(self, bot: Bot, message, users: list):
        self.bot = bot
